    ...
    >
      ...
      <template #header>
        ...
        >
          <div>
            CreMind AI
            <LoadImg class="cremind-icon-bar" :filename="'CreMind-logo-64.png'" :width="25" />
          </div>
            ...
            <ElButtonGroup>
              ...
              <ElTooltip content="Save this conversation" placement="top">
                <ElButton plain @click="handleSaveConversation">
                  <Icon icon="fluent:save-28-regular" :style="{ fontSize: '20px' }" />
                </ElButton>
              </ElTooltip>
      ...
      <!-- Chat component -->
      <Chat ref="chatRef" :chats="chats" @new-chat="newChat" v-model:blockSend="isStreaming" />
    </ElDialog>

    <!-- Chatting button -->
    <div class="button-chatting">
      <Icon icon="fluent:chat-12-filled" :style="{ fontSize: '50px' }" @click="onStartChatBox" />
      <LoadImg class="cremind-chatting" :filename="'CreMind-logo-64.png'" :width="15" />
    </div>
  </div>
</template>
...
const sendMessage = async (prompt: string, regenerate: boolean) => {
  ...
  const chain = new Chain('Conversation', llm, promptTemplate, null, null, null, true)
