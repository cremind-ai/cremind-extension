  ...
  if (data && data.topic === IPCTopicEnum.COMMUNICATION) {
    if (data.type === CommunicationMessageTypeEnum.GET_FEATURES) {
      fetch(`${import.meta.env.VITE_CREMIND_API!}/prompt/features`, {
        ...
        .then((response) => {
          if (!response.ok) {
            throw new CWException(Status.BACKEND_REQUEST_UNKNOWN_ERROR, 'Request unknown error')
          }
...
ipcHost.register(IPCTopicEnum.CONVERSATION, async (data: IPCMessageType, sendResponse) => {
    ...
    aiProvider
      ...
      .then(async (callback) => {
        callback((resData: AIResponseType) => {
          ...
          } else if (resData && resData.type === AIResponseTypeEnum.COMPLETE) {
            const payload = {
              message: resData.message,
