...
export class ChatGPT extends AIProvider {
  ...
  private async request(token: string, method: string, path: string, data?: unknown) {
  ...
  private async fetchModels(
    ...
    const resp = await this.request(token, 'GET', 'backend-api/models').then((r) => r.json())
    return resp.models
  }
  ...
  private async setConversationProperty(
    ...
    await this.request(token, 'PATCH', `backend-api/conversation/${conversationId}`, propertyObject)
  }

  private async genTitle(conversationId: string, messageId: string): Promise<string> {
  ...
  async conversation(
    ...
    return new Promise<(callback: (data: AIResponseType) => void) => void>((resolve, reject) => {
      ...
      resolve(async (callback: (data: AIResponseType) => void) => {
        ...
        const childId = uuid()
        const resp = await fetch(this.CHATGPT_URL + '/backend-api/conversation', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...
            if (message === '[DONE]') {
              ...
              if (this.conversationId) {
                if (args.deleteConversation) {
                  ...
                  if (!conversationId) {
                    const title = await this.genTitle(this.conversationId, this.messageId!)
